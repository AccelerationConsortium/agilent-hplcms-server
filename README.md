...
- **Shared-method-file constraint:** queued OpenLab runs reference the instrument method by path. Submitting another openlab-dispatch with a *different gradient* before earlier handed-off runs have acquired may re-edit files those runs still reference. One gradient per outstanding handoff batch; the server does not police this.
